...
def read_input(input):
    for line in input:
        line = re.sub("[^a-zA-Z0-9]", " ", line.lower()) # Apply regex to replace undesired characters
...
def main(separator='\t'): # default separator is a tab - which is \t - can change it to something else
    ...
    if words_in_line  > 2: # check if there are atleast 3 words in the line
        for i in rangeI(words_in_line):
            ...
            if i < words_in_line - 1: # bigrams in line = ungrams in line - 1
                print("%s%s%d" % (words[i] + " " + words[i + 1], "\t", 1)) # individual bigram count
                print("%s%s%d" % ("bigram_count", "\t", 1)) # total bigram count
            if i < words_in_line - 2: # trigrams in line = unigrams - 2
                print("%s%s%d" % (words[i] + " " + words[i + 1] + " " + words[i + 2], "\t", 1)) # individual trigram count
